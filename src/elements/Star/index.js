...
export default function Star({ value, height, width, spacing, className }) {
    ...
    if (decimals > 0  && value <= 5) {
        star.push(
        <div className='star' key={`starWithDecimal`} style={{ left: leftPos, height:height ,width: width * decimals - spacing, marginRight: spacing }}></div>
        )
    }
    ...
    for (let index = 0; index < 5; index++) {
        ...
        starPlaceholder.push(
            <div className='star placeholder' key={`starPlaceholder-${index}`} style={{ 
                left: index * width, 
                height: height, 
                width: width, 
